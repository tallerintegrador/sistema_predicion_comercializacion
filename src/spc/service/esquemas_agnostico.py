...
Ubicación **neutral** (capa de servicio) para romper la dependencia invertida: el motor
``spc.service.agnostico`` necesita estos modelos, y la capa de servicio no debe importar de
la capa de API. La API los sigue exponiendo vía ``spc.api.schemas.agnostico`` (re-export
delgado), de modo que el contrato HTTP no cambia.
...
(`SchemaSpec`) y envía **filas con columnas arbitrarias** (``rows``). El sistema entrena en
el momento el **algoritmo ganador** sobre esa data (`spc.models.automl`) y predice/mejora, de
modo que sirve a cualquier sector sin reentrenar el motor a mano.
