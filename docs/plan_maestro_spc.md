...
> Documento vivo. Versión 0.3 (Fase 0, 1 y 2a cerradas; 2a revisada y cerrada con rigor). Se actualiza al cierre de cada fase, dejando rastro en el registro de decisiones (ADR).
...
- **Sub-fases y entregables:**
  - **2a — Regresión (VENTAS):** objetivo `sales` transformado con **log1p** (el EDA muestra que reduce la asimetría de 7.36 → 0.41); **validación temporal sin fuga de futuro** (corte por fecha / `TimeSeriesSplit`); features de calendario, promoción, transacciones y rezagos. Comparar contra un **baseline** (media móvil / *naïve* estacional).
    - **[HECHO — 2026-06-13]** Se compararon 2 baselines + 5 regresores (Ridge, RandomForest, HistGradientBoosting, LightGBM, XGBoost) bajo validación temporal sin fuga + CV expanding. **Modelo de producción elegido por estabilidad** (no sólo por MAE de test): **HistGradientBoosting** (`regresion_v2`) — los tres boosters empatan en MAE de CV y HGB es el más estable (RMSE_std ≈ 11.8) y sólo depende de scikit-learn. Métricas TEST (unidades): **WAPE 14.2 · MAE 66.5 · RMSE 227.5 · RMSLE 0.388**, frente al mejor baseline (MAE 90.84 / RMSE 297.31): **MAE −26.8 %, RMSE −23.5 %**. El **MAPE (~32 %) se marca como inflado** por el 31 % de ceros y no se usa como métrica principal. El lineal (Ridge), tras montarse en pipeline propio (one-hot + estandarización), sigue no apto y se **retira de las tablas** con nota. Artefacto serializado con metadatos enriquecidos (criterio de selección, features, semilla, volúmenes de comparación vs. final) en `models/`; entrenado sobre todo el histórico etiquetado (≈2.98 M filas). Importancia de features (permutation, held-out) añadida al reporte. Detalle en `docs/reporte_regresion_2a.md`; decisiones en ADR `0002` (selección inicial) y `0003` (cierre con rigor). **Diferido (documentado):** intervalos de predicción y enfoque zero-inflated/two-part.
