...
> **Actualización (2026-06-20) — qué se implementó después de este documento.** Sobre la rama se añadieron, tras la Fase 3.7: (1) **persistencia incremental del corpus** (base SQLite local que acumula —deduplicado— el `history` de cada cliente; ADR-0011) y (2) un **frontend** web local. **Aclaración importante de honestidad:** esto **NO** es la "opción B / ajuste por cliente": **no se entrenó ni se ajustó ningún modelo con datos del cliente**. Solo se construyó la **acumulación** que *haría posible* ese ajuste más adelante por un **puente manual offline** (export → deduplicar → entrenar en GPU → reemplazar el artefacto). **El modelo sigue congelado** (ADR-0009) y `client_adjustment` **sigue `planned`** en el catálogo. **Nada se ha desplegado todavía** (la Fase 4 sigue pendiente). La "opción B" del §8, por tanto, **sigue abierta y sin medir**.
