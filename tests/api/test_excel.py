...
def _subir(client, ruta: str, contenido: bytes):
    """Sube un .xlsx al endpoint indicado (multipart)."""
    return client.post(ruta, files={"file": ("datos.xlsx", contenido, XLSX)})
    ...
    contenido = _xlsx(
        {
            ...
            "parameters": [["granularity", "horizon"], ["day", 5]],
        }
    )
    r_xlsx = _subir(client, "/sales/excel", contenido)
...
def test_equivalencia_descarga_y_sube_la_propia_plantilla(client):
    """La plantilla descargada (con su fila de ejemplo) se sube y predice sin tocarla."""
    contenido = client.get("/sales/template").content
    r = _subir(client, "/sales/excel", contenido)
    assert r.status_code == 200, r.text
...
def test_fecha_como_celda_datetime_equivale_a_iso(client, historico_contrato):
    ...
    for h in historico_contrato:
        ...
        filas.append(fila)
    contenido = _xlsx({"history": filas, "parameters": [["granularity", "horizon"], ["day", 3]]})
    r_xlsx = _subir(client, "/sales/excel", contenido)
    assert r_xlsx.status_code == 200, r_xlsx.text
    assert r_xlsx.json() == r_json.json()
...
def test_error_texto_en_columna_numerica(client, historico_contrato):
    ...
    contenido = _xlsx({"history": filas, "parameters": [["granularity", "horizon"], ["day", 5]]})
    r = _subir(client, "/sales/excel", contenido)
...
def test_error_hoja_faltante(client, historico_contrato):
    """Falta la hoja parameters de SALES → 422 indicando la hoja faltante."""
    contenido = _xlsx({"history": _history_filas(historico_contrato)})
    r = _subir(client, "/sales/excel", contenido)
    _assert_422_excel(r, "parameters")
...
def test_error_regla_de_contrato_horizon(client, historico_contrato):
    """horizon=0 (viola gt=0) se valida con el MISMO modelo → 422 mapeado a parameters."""
    contenido = _xlsx(
        {
            "history": _history_filas(historico_contrato),
            "parameters": [["granularity", "horizon"], ["day", 0]],
        }
    )
    r = _subir(client, "/sales/excel", contenido)
    _assert_422_excel(r, "parameters")
...
def test_error_archivo_ilegible(client):
    ...
    r = _subir(client, "/sales/excel", b"esto no es un excel")
    assert r.status_code == 422, r.text
